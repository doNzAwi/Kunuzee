...
// ───────────────────────────────────────────────────────────────
// FUNCTION 0: fixThankYouTitle — تثبيت عنوان صفحة الشكر ومنع أي glitch
// ───────────────────────────────────────────────────────────────
(function() {
    'use strict';

    function fixThankYouTitle() {
        var h1 = document.querySelector('.thanks_container h1');
        if (!h1) return;

        // لو فيه أي نص داخل الـ h1 (مش pseudo-elements)، امسحه
        var childNodes = Array.from(h1.childNodes);
        childNodes.forEach(function(node) {
            if (node.nodeType === 3) { // text node
                node.textContent = '';
            }
            if (node.nodeType === 1 && !node.matches('::after') && !node.matches('::before')) {
                // element node - check if it's not our pseudo-elements
                if (!node.classList.contains('thanks-subtitle')) {
                    node.style.display = 'none';
                }
            }
        });

        // تأكد إن الـ h1 نفسه ظاهر
    ...
    // شغل على طول
    fixThankYouTitle();
    setTimeout(fixThankYouTitle, 0);
    setTimeout(fixThankYouTitle, 50);
    setTimeout(fixThankYouTitle, 100);
    setTimeout(fixThankYouTitle, 200);
    setTimeout(fixThankYouTitle, 500);
    ...
    setInterval(function() {
        if (location.href !== lastUrl) {
            ...
            setTimeout(fixThankYouTitle, 300);
        }
    }, 300);
})();
