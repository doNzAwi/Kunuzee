...
// ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── //
// ───────────────────────────────────────────────────────────────
// FUNCTION 18: FAQ iPad Fix — iPad Mini (768×1024) & iPad Air (820×1180)
// ───────────────────────────────────────────────────────────────
(function() {
    'use strict';

    function fixFaqForIpad() {
        var width = window.innerWidth;
        var height = window.innerHeight;
        
        // iPad Mini: 768×1024 (portrait) أو 1024×768 (landscape)
        // iPad Air: 820×1180 (portrait) أو 1180×820 (landscape)
        var isIpadMini = (
            (width >= 760 && width <= 775 && height >= 1015 && height <= 1035) ||
            (width >= 1015 && width <= 1035 && height >= 760 && height <= 775)
        );
        
        var isIpadAir = (
            (width >= 810 && width <= 830 && height >= 1170 && height <= 1190) ||
            (width >= 1170 && width <= 1190 && height >= 810 && height <= 830)
        );
        
        var isIpadMiniAir = isIpadMini || isIpadAir;

        var accordion = document.querySelector('.szh-accordion');
        var faqContainer = accordion ? accordion.closest('.home_section_container') : null;
        
        if (!faqContainer) return;

        if (isIpadMiniAir) {
            faqContainer.classList.add('kunuzee-ipad-faq-mobile');
        } else {
            faqContainer.classList.remove('kunuzee-ipad-faq-mobile');
        }
    }

    fixFaqForIpad();
    
    window.addEventListener('resize', fixFaqForIpad);
    window.addEventListener('orientationchange', fixFaqForIpad);
