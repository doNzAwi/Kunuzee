...
// ───────────────────────────────────────────────────────────────
// FUNCTION 13: cleanThanksContent — إزالة أي نص ديناميكي من صفحة الشكر
// ───────────────────────────────────────────────────────────────
(function() {
    'use strict';

    function cleanThanksContent() {
        var content = document.querySelector('.thanks_content');
        if (!content) return;
        ...
        children.forEach(function(child) {
            ...
            // سيب الـ div الأول (اللي فيه SVG)
            if (tag === 'DIV' && child.querySelector('svg')) return;
            
            // سيب الـ h1
            if (tag === 'H1') return;
            
            // سيب الـ div.mt-6 (اللي فيه الزرار)
            if (tag === 'DIV' && child.classList.contains('mt-6')) return;
            
            // ✅ تأكد إن ده مش جزء من الفاتورة
            if (child.classList.contains('order_invoice_container')) return;
            if (child.querySelector('.order_invoice_container')) return;
            
            // اي حاجة تانية، امسحها
    ...
    cleanThanksContent();
    setTimeout(cleanThanksContent, 0);
    setTimeout(cleanThanksContent, 50);
    setTimeout(cleanThanksContent, 100);
    setTimeout(cleanThanksContent, 200);
    setTimeout(cleanThanksContent, 500);
    ...
    var observer = new MutationObserver(function(mutations) {
        ...
        mutations.forEach(function(mutation) {
            mutation.addedNodes.forEach(function(node) {
                if (node.nodeType === 1 && (
                    node.closest?.('.thanks_content') ||
                    node.classList?.contains('thanks_content')
                )) {
                    hasContent = true;
                }
            });
