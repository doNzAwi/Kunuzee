...
// ───────────────────────────────────────────────────────────────
// FUNCTION 16: fixSortListboxPosition — تثبيت قايمة الترتيب
// ───────────────────────────────────────────────────────────────
(function() {
    'use strict';

    var GAP = 4;
    var FIXED_CLASS = 'kunuzee-listbox-fixed';
    ...
    function applyFixedPosition(options, button) {
        var pos = getButtonRect(button);
        options.classList.add(FIXED_CLASS);
        options.style.setProperty('--kunuzee-listbox-top', pos.top + 'px');
        options.style.setProperty('--kunuzee-listbox-right', pos.right + 'px');
        options.style.setProperty('--kunuzee-listbox-width', pos.width + 'px');
    }

    function removeFixedPosition(options) {
        options.classList.remove(FIXED_CLASS);
        options.style.removeProperty('--kunuzee-listbox-top');
        options.style.removeProperty('--kunuzee-listbox-right');
        options.style.removeProperty('--kunuzee-listbox-width');
    }
    ...
    function handleListbox() {
        ...
        if (window.getComputedStyle(options).display !== 'none') {
            applyFixedPosition(options, button);
        } else {
            removeFixedPosition(options);
        }
    }
    ...
    var observer = new MutationObserver(function(mutations) {
        ...
        mutations.forEach(function(mutation) {
            mutation.addedNodes.forEach(function(node) {
                if (node.nodeType === 1 && node.id && node.id.indexOf('headlessui-listbox-options') !== -1) {
                    needsUpdate = true;
                }
            });
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('[id*="headlessui-listbox-button"]')) {
            setTimeout(handleListbox, 0);
            setTimeout(handleListbox, 50);
            setTimeout(handleListbox, 150);
        }
    });
